...
- **Линеаризация** зависимостей (DFS по требованиям миксина) — каждый миксин применяется в цепочке один раз; ромбовидное наследование дедуплицируется по identity миксина.
- **Мемоизация** применений по паре (миксин, база) — повторное применение к той же базе возвращает закэшированный класс.
...
1. **Runtime-хелпер** (`mixinChain`, `AnyConstructor`, `ClassStatics`, `Symbol.hasInstance`, имена классов) в runtime-входе пакета — портирование линеаризации/дедупликации/мемоизации из `typescript-mixin-class`. Юнит-тесты на цепочки, ромб, instanceof.
2. **Трансформация mixin-класса** (режим «печать + репарс»): генерация interface + фабрики + const; перенос тела; сбор зависимостей из `implements`. Юнит-тесты уровня `transformSourceFile` (по образцу `tests/source-transform.t.ts`).
