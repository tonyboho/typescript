...
| 7.19 | A user's **own `static new`** on a construction CLASS or CONSUMER **overrides** the generated factory (the transform skips generating its own — `hasStaticNew`; the user signature governs `.new(...)` call sites); on a `@mixin` the name is RESERVED — see §11.12 | ✅ | `reserved-static-members.t.ts` |
...
| 11.12 | **Reserved framework statics on a `@mixin`**: a user `static mix` or `static new` is rejected (native TS990004-family message naming the reserved static, both planes) — `.mix(base)` is installed on every mixin value, `.new(...)` is the construction protocol. NOT restricted elsewhere: a consumer's `static mix` is an ordinary user static (the framework `.mix` lives on mixin values only and is `Omit`ted from the consumer's inherited statics bag — carrying it was a type lie AND blocked the user static with TS2417), and a consumer's/class's `static new` overrides the generated factory (§7.19) | ✅ | `reserved-static-members.t.ts`, `source-transform-consumer-emit.t.ts` (the `Omit … "mix"` pin) |
