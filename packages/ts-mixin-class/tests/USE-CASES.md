...
| 7.11 | `fillMissedInitializersWith` option (default `"undefined"`): a public construction field with no source initializer is filled (`undefined!`/`null!`, type not widened) for a stable object shape; `"nothing"` opts out | ✅ | `construction-fill-missed-initializers.t.ts` |
