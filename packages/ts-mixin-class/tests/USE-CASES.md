...
| 2.14 | **Member-KIND override guard (TS990010)**: a class FIELD shadowing a mixin ACCESSOR is rejected in both planes and under BOTH class-field semantics (the checker's own TS2610 cannot see accessor-ness through the generated interface); an ACCESSOR over a mixin FIELD is rejected **only under define semantics** — a deliberate deviation from plain TS2611, since with `useDefineForClassFields: false` the field assignment fires the overriding setter (the reactive-property pattern). Also covers mixin-vs-mixin overlaps in one `implements` list (first-listed = nearest layer) and transitive LOCAL `extends` chains; `.d.ts` mixins (no program AST) are skipped | ✅ | `member-kind-collisions.t.ts` |
